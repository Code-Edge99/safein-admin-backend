...
export class ControlLogsController {
  ...
  async getRecentLogs(@Req() req: AuthenticatedAdminRequest, @Query('limit') limit?: number): Promise<ControlLogResponseDto[]> {
  ...
  async findOne(@Req() req: AuthenticatedAdminRequest, @Param('id') id: string): Promise<ControlLogResponseDto> {
