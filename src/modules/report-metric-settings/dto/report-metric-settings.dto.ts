...
export class ReportMetricSettingsDto {
  ...
  @ApiProperty({ description: '직원 위험도 - 위험 판정 안정 점수 미만 기준(%)', example: 60 })
  @Type(() => Number)
  @IsNumber({ maxDecimalPlaces: 1 })
  @Min(0)
  @Max(100)
  employeeRiskComplianceDangerBelow: number;

  @ApiProperty({ description: '직원 위험도 - 보통 판정 안정 점수 미만 기준(%)', example: 80 })
  @Type(() => Number)
  @IsNumber({ maxDecimalPlaces: 1 })
  @Min(0)
  @Max(100)
  employeeRiskComplianceWarningBelow: number;

  @ApiProperty({ description: '직원 위험도 - 위험 판정 구역 위반 건수 이상 기준', example: 5 })
  @Type(() => Number)
  @IsInt()
  @Min(0)
  employeeRiskZoneViolationsDangerAbove: number;

  @ApiProperty({ description: '직원 위험도 - 보통 판정 구역 위반 건수 이상 기준', example: 2 })
  @Type(() => Number)
  @IsInt()
  @Min(0)
  employeeRiskZoneViolationsWarningAbove: number;

  @ApiProperty({ description: '직원 위험도 - 위험 판정 총 차단 건수 이상 기준', example: 20 })
  @Type(() => Number)
  @IsInt()
  @Min(0)
  employeeRiskTotalBlocksDangerAbove: number;

  @ApiProperty({ description: '직원 위험도 - 보통 판정 총 차단 건수 이상 기준', example: 10 })
  @Type(() => Number)
  @IsInt()
  @Min(0)
  employeeRiskTotalBlocksWarningAbove: number;

  @ApiProperty({ description: '직원 위험도 점수 - 안정 점수 항목 가중치', example: 1 })
  @Type(() => Number)
  @IsNumber({ maxDecimalPlaces: 2 })
  @Min(0)
  @Max(10)
  employeeRiskComplianceWeight: number;

  @ApiProperty({ description: '직원 위험도 점수 - 구역 위반 항목 가중치', example: 1 })
  @Type(() => Number)
  @IsNumber({ maxDecimalPlaces: 2 })
  @Min(0)
  @Max(10)
  employeeRiskZoneViolationsWeight: number;

  @ApiProperty({ description: '직원 위험도 점수 - 총 차단 항목 가중치', example: 1 })
  @Type(() => Number)
  @IsNumber({ maxDecimalPlaces: 2 })
  @Min(0)
  @Max(10)
  employeeRiskTotalBlocksWeight: number;

  @ApiProperty({ description: '직원 위험도 점수 - 위험 판정 최소 점수', example: 100 })
  @Type(() => Number)
  @IsNumber({ maxDecimalPlaces: 1 })
  @Min(0)
  @Max(1000)
  employeeRiskDangerScoreMin: number;

  @ApiProperty({ description: '직원 위험도 점수 - 보통 판정 최소 점수', example: 60 })
  @Type(() => Number)
  @IsNumber({ maxDecimalPlaces: 1 })
  @Min(0)
  @Max(1000)
  employeeRiskWarningScoreMin: number;
